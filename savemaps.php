...
$sql ="SELECT * FROM 3e_gps WHERE checkid = $newid ";
$result = mysql_query($sql);

if(mysql_num_rows($result) < 1 )
{
  $sql="INSERT INTO 3e_gps(id,nametutor,lat,lng,checkid) VALUES('','$locatName','$lat','$lng','$newid')";
}
else
{
...
<table width="100%" border="0" cellpadding="3" cellspacing="0" style="border:1px solid #CCC;background-color:#E4E4E4;">
  ...
  $rsMapsGoo=mysql_query("SELECT * FROM 3e_gps WHERE 3e_gps.checkid = '$newid'");
  while($showMapsGoo=mysql_fetch_array($rsMapsGoo)){
  ?>
  <tr>
    <td><a href="showmaps.php?mapsId=<?=$showMapsGoo['id']?>" target="_blank"><?=$showMapsGoo['nametutor']?></a></td>
